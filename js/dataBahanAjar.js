var app = new Vue({
  ...
  data: {
    ...
    isFilterOpen: false,
    filterOpt: {
      regional: "", kategori: "", harga: "", sortName: "", sortSafety: "", sortQty: ""
    },
  ...
  computed: {
    ...
    filteredStok() {
      ...
      if (this.filterOpt.regional !== "") result = result.filter(item => item.upbjj === this.filterOpt.regional);
      if (this.filterOpt.kategori !== "") result = result.filter(item => item.kategori === this.filterOpt.kategori);

      // Sorting (Urutkan Data)
      if (this.filterOpt.sortName === "az") result.sort((a, b) => a.judul.localeCompare(b.judul));
      else if (this.filterOpt.sortName === "za") result.sort((a, b) => b.judul.localeCompare(a.judul));

      if (this.filterOpt.harga === "tinggi") result.sort((a, b) => b.harga - a.harga);
      else if (this.filterOpt.harga === "rendah") result.sort((a, b) => a.harga - b.harga);

      if (this.filterOpt.sortSafety === "min") result.sort((a, b) => a.safety - b.safety);
      else if (this.filterOpt.sortSafety === "max") result.sort((a, b) => b.safety - a.safety);

      if (this.filterOpt.sortQty === "min") result.sort((a, b) => a.qty - b.qty);
      else if (this.filterOpt.sortQty === "max") result.sort((a, b) => b.qty - a.qty);
  ...
  methods: {
    ...
    simpanTrackingBaru() {
      if (!this.formTracking.nomorDO) {
        alert("Nomor DO wajib diisi!"); return;
      }
      ...
      const newData = {
        ...
        perjalanan: [
          { waktu: new Date().toISOString().slice(0, 19).replace('T', ' '), keterangan: "Pesanan Dibuat (Order Confirmed)" }
        ]
      };
    ...
    simpanDataStok() {
      ...
      this.formStok = { kode: "", judul: "", kategori: "", upbjj: "", lokasiRak: "", harga: null, qty: null, safety: null, catatanHTML: "" };
    },
    ...
    resetFilter() {
      this.filterOpt = { regional: "", kategori: "", harga: "", sortName: "", sortSafety: "", sortQty: "" };
    },
